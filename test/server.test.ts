...
describe('MCP server', () => {
    it('search_commons handler returns structuredContent with results (live Commons API)', async () => {
        ...
        const tryQueries = async (queries: string[], limit = 1) => {
            for (const q of queries) {
                ...
                if (Array.isArray(r.structuredContent.results) && r.structuredContent.results.length) {
                    return r;
                }
            }
        ...
        if (!Array.isArray(res.structuredContent.results) || res.structuredContent.results.length === 0) {
            // basic shape checks
            if (!Array.isArray(res.structuredContent.results)) throw new Error('expected results array');
        ...
        if (first.imageUrl && res.content[0].text.indexOf(first.imageUrl) === -1) {
            throw new Error('textual content does not include imageUrl from the first result');
        }
    });
    ...
    it('search_commons snapshot for "Scarlett Johansson"', async () => {
        const { handlers } = createMcp();
        const res = await handlers.search_commons({ query: 'Scarlett Johansson', limit: 3 });
